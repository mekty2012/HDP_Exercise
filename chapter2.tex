...
\end{customExercise}
\begin{proof}
  From the assumption, we have $d \le c$ for some $c$.
  We will do similar, where we instead need to show that
  \[
    P\left( d_i > C \frac{\log n}{\log \log n} \right) \le 0.9/n.
  \]
  Again by Theoerm 2.3.1, we have the bound
  \begin{align*}
    &P\left(d_i > \frac{C \log n}{\log \log n}\right)
    \\
    \le &e^{-d} \left( \frac{e d \log \log n}{C \log n} \right)^{C \log n / \log \log n}
    \\
    \le &e^{\frac{C \log n}{\log \log n}-d} \left( \frac{c \log \log n}{C \log n} \right)^{C \log n / \log \log n}
  \end{align*}
  Let's assume $C \ge c$ for simplicity. This allows us to bound
  \[
    P \left( d_i > \frac{C \log n}{\log \log n} \right) \le e^{\frac{C \log n}{\log \log n} - d} \left( \frac{\log \log n}{\log n} \right)^{C \log n / \log \log n}
  \]
  Now, write $t = \log n / \log \log n$, and let $g(t) = (1/t)^t$, which allows us to see
  \begin{align*}
    g'(t) &= (1/t)^t \left( \log (1/t) - 1 \right),
    \\
    g'(t) &= 0 \Leftrightarrow t = 1/e,
    \\
    g''(1/e) &= - e^{1/e + 1}. 
  \end{align*}
  So this function attains its maximum at $t = 1/e$, with the value $e^{1/e}$. 
  Applying this to our case, we can obtain
  \[
    P\left( d_i > \frac{C \log n}{\log \log n} \right) \le e^{\frac{C \log n}{\log \log n} - d + C/e}
  \]
